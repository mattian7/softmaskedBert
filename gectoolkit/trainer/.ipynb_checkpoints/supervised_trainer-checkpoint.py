...
class SupervisedTrainer(AbstractTrainer):
    ...
    def _train_batch(self, batch, bert,embedding, is_display=False):
        
        batch_loss = self.model(batch, self.dataloader, bert, embedding, is_display=is_display)
        return batch_loss

    def _eval_batch(self, batch, bert, embedding, is_display=False):
        ...
        '''
        test_out, target = self.model.model_test(batch, self.dataloader, bert, embedding, is_display)
    ...
    def _train_epoch(self, bert,embedding):
        ...
        for batch_idx, _ in enumerate(self.dataloader.load_data(DatasetType.Train)):
            ...
            batch_loss = self._train_batch(batch, bert,embedding, is_display=False)
            ...
            batch_loss["loss"].backward(retain_graph=True)
    ...
    def fit(self):
        ...
        bert = BertModel.from_pretrained(self.config["pretrained_model_path"])
        ...
        for epo in tqdm(range(self.start_epoch, epoch_nums), desc='train process '):
            ...
            self.model.train()
            loss_total, train_time_cost = self._train_epoch(bert,embedding)
            ...
            if epo > 0 and (epo % self.test_step == 0) or (epo > epoch_nums - 5):
                _, valid_det_f1, valid_cor_f1, valid_total, valid_time_cost = self.evaluate(bert, embedding, DatasetType.Valid)

                self.logger.info(
                    "---------- valid total [%d] | valid det f1 [%2.3f] | valid cor f1 [%2.3f] | valid time %s" \
                    % (valid_total, valid_det_f1, valid_cor_f1, valid_time_cost))   
                
                predict_out, test_det_f1, test_cor_f1, test_total, test_time_cost = self.evaluate(bert, embedding, DatasetType.Test)
        ...
        self.logger.info('''training finished.
                         ...
                         % (self.best_valid_det_f1, self.best_valid_cor_f1))

    def evaluate(self, bert, embedding, eval_set):
        """evaluate model.
        ...
        for batch_idx in tqdm(range(batch_nums), desc='test {}set'.format(eval_set)):
            #if batch_idx > 300: continuerecover_num

            batch = self.dataloader.load_next_batch(eval_set)
            pred_out, batch_det_f1, batch_cor_f1 = self._eval_batch(batch, bert, embedding, is_display = False)
    ...
    def test(self):
        ...
        for batch in self.dataloader.load_data(DatasetType.Test):
            pred_out, batch_det_f1, batch_cor_f1 = self._eval_batch(batch, bert, embedding)
            det_f1 += np.sum(batch_det_f1)
            cor_f1 += np.sum(batch_cor_f1)
            eval_total += len(batch_det_f1)
            predict_out += pred_out
