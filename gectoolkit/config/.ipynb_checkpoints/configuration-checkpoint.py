...
class Config(object):
    ...
    def __init__(self, model_name=None, dataset_name=None, language_name=None, config_dict={}):
        """
        Args:
        ...
        self._init_external_config(model_name, dataset_name, language_name, config_dict)
        # load cmd line
        self._load_cmd_line()
    ...
    def _update(self, external_config):
        #print('internal_config_dict', self.internal_config_dict, external_config)
        for key in external_config:
            self.final_config_dict[key] = external_config[key]
        #print(self.final_config_dict); #exit()

    def _load_internal_config(self):        
        dir = os.path.dirname(os.path.realpath(__file__))
        config_path = os.path.join(dir, 'config.json')
        self.internal_config_dict = read_json_data(config_path)
        # print('self.internal_config_dict', self.internal_config_dict)

    def _init_external_config(self,model_name, dataset_name, language_name, config_dict):
        self.external_config_dict['model'] = model_name
        self.external_config_dict['language_name'] = language_name
        self.external_config_dict['dataset'] = dataset_name
        self.external_config_dict.update(config_dict)
        # print('self.external_config_dict', self.external_config_dict)
    ...
    def _load_cmd_line(self):
        ...
        if len(unrecognized_args) > 0:
            logger = getLogger()
            logger.warning('command line args [{}] will not be used in gectoolkit'.format(' '.join(unrecognized_args)))
