...
class GECDataLoader(AbstractDataLoader):
    ...
    def __build_batch(self, batch_data):
        ...
        for data in batch_data:
            ...
            label = [w for w in data['label'].strip().split()][:self.max_input_len]
            
            sor_list = self.pretrained_tokenzier.convert_tokens_to_ids([SpecialTokens.CLS_TOKEN])
            for token in source:
                sor_list.append(self.pretrained_tokenzier.convert_tokens_to_ids(token))
            sor_list += self.pretrained_tokenzier.convert_tokens_to_ids([SpecialTokens.SEP_TOKEN])

            tag_list = self.pretrained_tokenzier.convert_tokens_to_ids([SpecialTokens.CLS_TOKEN])
            for token in target:
                tag_list.append(self.pretrained_tokenzier.convert_tokens_to_ids(token))
            tag_list += self.pretrained_tokenzier.convert_tokens_to_ids([SpecialTokens.SEP_TOKEN])

            lab_list = list()
            lab_list.append(0)
            for l in label:
                lab_list.append(int(l))
            lab_list.append(0)
            
            mask_list = [1]*len(sor_list)
            ...
            label_list_batch.append(lab_list)
            mask_list_batch.append(mask_list)
        ...
        return {
            ...
            "label_list_batch":label_list_batch,
            "mask_list_batch":mask_list_batch
        }
